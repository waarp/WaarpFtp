...
/**
 * Third CODEC :<br>
 * - encode/decode : takes {@link DataBlock} and transforms it to a
 * {@link DataBlock}<br>
...
public class FtpDataStructureCodec extends SimpleChannelHandler {
     ...
     *
     * In addition to different representation types, FTP allows the structure
     * of a file to be specified. Three file structures are defined in FTP:
     *
     * file-structure, where there is no internal structure and the file is
     ...
     * pages.
     *
     * FileInterface-structure is the default to be assumed if the STRUcture
     * command has not been used but both file and record structures must be
     * accepted for "text" files (i.e., files with TYPE ASCII or EBCDIC) by all
     * FTP implementations. The structure of a file will affect both the
     * transfer mode of a file (see the Section on Transmission Modes) and the
     * interpretation and storage of the file.
     *
     * The "natural" structure of a file will depend on which host stores the
     * file. A source-code file will usually be stored on an IBM Mainframe in
     * fixed length records but on a DEC TOPS-20 as a stream of characters
     ...
     * one site to recognize the other's assumptions about the file.
     *
     ...
     * record-structure to a host which is file oriented, then that host should
     * apply an internal transformation to the file based on the record
     * structure. Obviously, this transformation should be useful, but it must
     * also be invertible so that an identical file may be retrieved using
     * record structure.
     *
     * In the case of a file being sent with file-structure to a record-oriented
     * host, there exists the question of what criteria the host should use to
     * divide the file into records which can be processed locally. If this
     * division is necessary, the FTP implementation should use the end-of-line
     * sequence,
     *
     * <CRLF> for ASCII, or <NL> for EBCDIC text files, as the delimiter. If an
     * FTP implementation adopts this technique, it must be prepared to reverse
     ...
     * Files of this type are sometimes known as "random access files" or even
     * as "holey files". In these files there is sometimes other information
     * associated with the file as a whole (e.g., a file descriptor), or with a
     * section of the file (e.g., page access controls), or both. In FTP, the
     * sections of the file are called pages.
     *
     * To provide for various page sizes and associated information, each page
     * is sent with a page header. The page header has the following defined
     * fields:
     *
     * Header Length
     *
     * The number of logical bytes in the page header including this byte. The
     * minimum header length is 4.
     *
     * Page Index
     *
     * The logical page number of this section of the file. This is not the
     * transmission sequence number of this page, but the index used to identify
     * this page of the file.
     *
     * Data Length
     *
     * The number of logical bytes in the page data. The minimum data length is
     * 0.
     *
     * Page Type
     *
     ...
     * Optional Fields
     *
     * Further header fields may be used to supply per page control information,
     * for example, per page access control.
     *
     * All fields are one logical byte in length. The logical byte size is
     * specified by the TYPE command. See Appendix I for further details and a
     * specific case at the page structure.
     *
     * A note of caution about parameters: a file must be stored and retrieved
     * with the same parameters if the retrieved version is to
     *
     * be identical to the version originally transmitted. Conversely, FTP
