...
/**
 * Example of FTP Server using simple authentification (XML File based), and standard Directory and File implementation
 * (Filesystem based).
 * 
 * @author fbregier
 *
...
public class SimpleGatewayFtpServer {
	...
	public static void main(String[] args) {
		...
		FtpInternalLoggerFactory.setDefaultFactory(new Slf4JLoggerFactory(Level.WARN));
		logger = FtpInternalLoggerFactory.getLogger(SimpleGatewayFtpServer.class);
		String config = args[0];
		FileBasedConfiguration configuration = new FileBasedConfiguration(SimpleGatewayFtpServer.class, 
				SimpleBusinessHandler.class, FileSystemBasedDataBusinessHandler.class);
		if (! configuration.setConfigurationFromXml(config)) {
			System.err.println("Bad configuration");
			return;
		}
		// Init according JDK
		FileBasedDir.initJdkDependent(new FilesystemBasedFtpDirJdk6());
		// Start server.
