...
public class FileBasedConfiguration extends FilesystemBasedFtpConfiguration {
	...
	private static final FtpInternalLogger logger =
        FtpInternalLoggerFactory.getLogger(FileBasedConfiguration.class);
	...
	public boolean setConfigurationFromXml(String filename) {
		...
		try {
			this.setBaseDirectory(FilesystemBasedFtpDir.normalizePath(file.getCanonicalPath()));
		...
		if (node != null) {
			this.deleteOnAbort = (Integer.parseInt(node.getText()) == 1) ? true : false;
		}
		...
		if (node == null) {
			logger.error("Unable to find Authentification file in Config file: "+filename);
			return false;
		}
		...
		try {
			...
			logger.error("Unable to read the XML Authentification file: "+fileauthent,e);
			return false;
		}
		if (document == null) {
			logger.error("Unable to read the XML Authentification file: "+fileauthent);
			return false;
		}
	...
	public SimpleAuth getSimpleAuth(String user) {
		return this.authentifications.get(user);
	}
	/**
