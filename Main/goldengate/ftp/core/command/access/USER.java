 ...
 * USER command
 * @author frederic
 * goldengate.ftp.core.command.access USER
 * 
 */
public class USER extends AbstractCommand {
	 ...
	 */
	@Override
	public void exec() throws Reply501Exception, Reply421Exception, Reply530Exception {
		if (!this.hasArg()) {
		...
		try {
			nextCommandReply = this.getFtpSession().getFtpAuth().setUser(username);
