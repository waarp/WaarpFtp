 ...
 * AbstractCommand.java goldengate.ftp.core.command GoldenGateFtp
 * frederic
 */
...
public abstract class AbstractCommand {
	...
	private static final FtpInternalLogger logger =
        FtpInternalLoggerFactory.getLogger(AbstractCommand.class);
	 ...
	 * @exception FtpCommandAbstractException in case of an FTP Error occurs
	 */
	...
	public boolean isNextCommandValid(AbstractCommand newCommand) {
		Class newClass = newCommand.getClass();
		...
		if (this.extraNextCommand != null) {
			if (this.extraNextCommand.command == newClass) {
				logger.debug("VALID {} after {} since extra next command",newCommand.command,this.command);
				return true;
			}
			if ((this.code.nextValids != null) && (this.code.nextValids.length > 0)) {
				for (int i = 0; i < this.code.nextValids.length; i++) {
					if (this.code.nextValids[i] == newClass) {
						logger.debug("VALID {} after {} since next command",newCommand.command,this.command);
						return true;
					}
				}
			}
			logger.debug("NOT VALID {} after {}",newCommand.command,this.command);
			return false;			
		}
		if ((this.code.nextValids == null) || (this.code.nextValids.length == 0)) {
			// Any command is allowed
			logger.debug("VALID {} after {} since all valid",newCommand.command,this.command);
			return true;
		}
		for (int i = 0; i < this.code.nextValids.length; i++) {
			if (this.code.nextValids[i] == newClass) {
				logger.debug("VALID {} since next command {}",newCommand.command,this.command);
				return true;
			}
		}
		logger.debug("DEFAULT NOT VALID {} after {}",newCommand.command,this.command);
		return false;
	}
	 ...
	 * @param argx
	 * @return the integer
	 * @throws FtpInvalidArgumentException if the argument is not an integer
	 */
	...
	/**
	 * Set the previous command as the new current command (used after a incorrect
	 * sequence of commands or unknown command). Also clear the Restart object.
	 *
	 */
