 ...
 * NLST command
 * @author frederic
 * goldengate.ftp.core.command.service NLST
 * 
 */
	...
	public void exec() throws FtpCommandAbstractException {
		...
		if (! this.hasArg()) {
			...
			if (path.startsWith("-l") || path.startsWith("-L")) {
				...
				if (paths.length > 1) {
					files = this.getFtpSession().getFtpDir().listFull(paths[1],true);					
				} else {
					files = this.getFtpSession().getFtpDir().listFull(this.getFtpSession().getFtpDir().getPwd(),true);
				}
			} else {
				files = this.getFtpSession().getFtpDir().list(path);
			}
