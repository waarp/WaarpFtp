 ...
 * STRU command
 * @author frederic
 * goldengate.ftp.core.command.parameter STRU
 * 
 */
public class STRU extends AbstractCommand {
	...
	public void exec() throws Reply501Exception, Reply504Exception {
		...
		if (! this.hasArg()) {
			this.getFtpSession().getDataConn().setStructure(FtpArgumentCode.TransferStructure.FILE);
			this.getFtpSession().setReplyCode(FtpReplyCode.REPLY_200_COMMAND_OKAY,
					"Structure set to "+FtpArgumentCode.TransferStructure.FILE.name());
			return;
		}
		FtpArgumentCode.TransferStructure transferStructure;
		try {
			transferStructure = FtpArgumentCode.getTransferStructure(this.getArg().charAt(0));
		} catch (FtpInvalidArgumentException e) {
			throw new Reply501Exception("Unrecognize Structure: "+this.getArg());
		}
		if (transferStructure == FtpArgumentCode.TransferStructure.FILE) {
			...
			throw new Reply504Exception("Structure not implemented: "+transferStructure.name());
		}
		this.getFtpSession().setReplyCode(FtpReplyCode.REPLY_200_COMMAND_OKAY,
