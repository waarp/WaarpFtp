 ...
 * PASV command
 * @author frederic
 * goldengate.ftp.core.command.parameter PASV
 * 
 */
public class PASV extends AbstractCommand {
	...
	public void exec() throws Reply425Exception, Reply501Exception {
		...
		for (int i = 1; i <= FtpInternalConfiguration.RETRYNB ; i++) {
			int newport = FtpDataAsyncConn.getNewPassivePort(this.getConfiguration());
			if (newport == -1) {
			...
			try {
				...
				logger.warn("Pasv refused at try: "+i+" with port:  since {}"+newport,e.getMessage());
			}
		}
		if (! isInit) {
		...
		InetSocketAddress local = this.getFtpSession().getDataConn().getLocalAddress();
		String slocal = "Entering Passive Mode ("+FtpChannelUtils.getAddress(local)+")";
		InetAddress remote =
			this.getFtpSession().getDataConn().getRemoteAddress().getAddress();
		...
		this.getFtpSession().setReplyCode(FtpReplyCode.REPLY_227_ENTERING_PASSIVE_MODE,
				slocal);
