 ...
 * EPSV command
 * 
 * @author frederic goldengate.ftp.core.command.parameter EPSV
 * 
 */
public class EPSV extends AbstractCommand {
    ...
    public void exec() throws Reply425Exception, Reply501Exception {
        ...
        for (int i = 1; i <= FtpInternalConfiguration.RETRYNB; i ++) {
            newport = FtpDataAsyncConn.getNewPassivePort(this
                    .getConfiguration());
            if (newport == -1) {
        ...
        this.getFtpSession().setReplyCode(
                FtpReplyCode.REPLY_229_ENTERING_PASSIVE_MODE,
                "Entering Extended Passive Mode (|||" + newport + "|)");
        logger.info("EPSV: answer ready on " + slocal);
        /*
         * Could be:this.getFtpSession().setReplyCode(FtpReplyCode.
