 ...
 * STAT command
 * @author frederic
 * goldengate.ftp.core.command.service STAT
 * 
 */
public class STAT extends AbstractCommand {
	...
	public void exec() throws FtpCommandAbstractException {
		...
		if (! this.hasArg()) {
			...
			message += "\nControl: "+FtpChannelUtils.nbDataChannels(this.getConfiguration())+
			" Data: "+FtpChannelUtils.nbDataChannels(this.getConfiguration())+
			" Binded: "+this.getConfiguration().getFtpInternalConfiguration().getNbBindedPassive();
			message += "\nEnd of Status";
			this.getFtpSession().setReplyCode(FtpReplyCode.REPLY_211_SYSTEM_STATUS_REPLY,message);
		} else {
			// List of files from path
			path = this.getArg();
			List<String> filesInfo = this.getFtpSession().getFtpDir().listFull(path,true);
			...
			this.getFtpSession().setReplyCode(FtpReplyCode.REPLY_212_DIRECTORY_STATUS,message);
		}
	}
