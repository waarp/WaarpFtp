 ...
 * 
 * @author frederic goldengate.ftp.core.command.service SITE
 * 
 */
public class SITE extends AbstractCommand {
    ...
    public void exec() throws FtpCommandAbstractException {
        ...
        if (FtpCommandCode.isExtensionCommand(command.getCode())) {
            ...
            // time two commands
            if (this.getFtpSession().getDataConn().getFtpTransferControl()
                    .isFtpTransferExecuting()) {
                throw new Reply503Exception(
                        "Previous transfer command is not finished yet");
            }
        } else {
            throw new Reply502Exception("Command not implemented: " +
                    this.getArg());
        }
        ...
        if (this.getFtpSession().getCurrentCommand()
                .isNextCommandValid(command)) {
            ...
            command.setArgs(this.getFtpSession(), this.getArg(), null,
