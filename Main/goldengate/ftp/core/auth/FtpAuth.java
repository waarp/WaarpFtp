...
/**
 * Base of Ftp Authentification with root directory from authentification
 * (usually /user/account). No mount point is considered here (see {@link FilesystemBasedFtpAuth}).
 * @author fbregier
 *
 */
public abstract class FtpAuth {
	 ...
	 */
	protected abstract FtpNextCommandReply setBusinessUser(String user) throws Reply421Exception, Reply530Exception;
	/**
	 ...
	 * @throws Reply530Exception if there is a problem during the authentification
	 */
	public FtpNextCommandReply setUser(String user) throws Reply421Exception, Reply530Exception {
	 ...
	 * @throws Reply530Exception if there is a problem during the authentification
	 */
	protected abstract FtpNextCommandReply setBusinessPassword(String password) throws Reply421Exception, Reply530Exception;
	/**
	 ...
	 * @throws Reply530Exception if there is a problem during the authentification
	 */
	public FtpNextCommandReply setPassword(String password) throws Reply421Exception, Reply530Exception {
	 ...
	 * @throws Reply530Exception if there is a problem during the authentification
	 */
	protected abstract FtpNextCommandReply setBusinessAccount(String account) throws Reply421Exception, Reply530Exception, Reply502Exception;
	/**
	 ...
	 */
	public FtpNextCommandReply setAccount(String account) throws Reply421Exception, Reply530Exception, Reply502Exception {
	...
	/**
	 * Set the Auth to Identified or Not
	 * @param isIdentified
	 */
	...
	/**
	 * Is the current Authentification OK for full identification.
	 * It must be true after a correct sequence of identification:
	 * At most, it is true when setAccount is OK.
	 * It could be positive before (user name only, user+password only).<br>
	 * In the current implementation, as USER+PASS+ACCT are nedded, it will be true only after a correct ACCT.
	...
	/**
	 * 
	 * @return the root relative path from authentification if any or null if the default is used 
	 * (default is /user or /user/account)
	 * @exception Reply421Exception if the business root is not available
	 */
	protected abstract String setBusinessRootFromAuth() throws Reply421Exception;
	/**
	 * Set the root relative Path from current status of Auth (should be the highest level for the current authentification).
	 * If setBusinessRootFromAuth returns null, by default set /user or /user/account.
	 * @exception Reply421Exception if the business root is not available
	 */
	private void setRootFromAuth() throws Reply421Exception {
		...
		if (this.rootFromAuth == null) {
			if (this.account == null) {
				this.rootFromAuth = FtpDir.SEPARATOR+user;
			} else {
				this.rootFromAuth = FtpDir.SEPARATOR+user+FtpDir.SEPARATOR+account;
			}
		}
	}
	/**
	...
	/**
	 * Return the relative path for this account according to the Business (without true root of mount).<br>
