...
public class FtpTransferExecutor implements Runnable {
    ...
    private void runNextCommand() throws InterruptedException {
        if (FtpCommandCode
                .isStoreLikeCommand(this.executeTransfer.getCommand())) {
            // The command is implicitly done by receiving message
            logger.debug("Command launch: {} {}", this.executeTransfer
                    .getCommand(), this.session);
            this.waitForCommand();
            // Store set end
            this.session.getDataConn().getFtpTransferControl()
                    .setEndOfTransfer();
            logger.debug("Command finished: {} {}", this.executeTransfer
                    .getCommand(), this.session);
        } else if (FtpCommandCode.isListLikeCommand(this.executeTransfer
                .getCommand())) {
            // No wait for Command since the answer is already there
            logger.debug("Command launch: {} {}", this.executeTransfer
                    .getCommand(), this.session);
            List<String> list = this.executeTransfer.getInfo();
            ...
            try {
                status = this.session.getDataConn().getDataNetworkHandler()
                        .writeMessage(message);
            } catch (FtpNoConnectionException e) {
                logger.error("No Connection but should not be!", e);
            }
            // Set status for check, no wait for the command
            this.executeTransfer.setStatus(status);
            // must explicitly set the end and no wait
            this.session.getDataConn().getFtpTransferControl()
                    .setEndOfTransfer();
            logger.debug("Command finished: {} {}", this.executeTransfer
                    .getCommand(), this.session);
        } else if (FtpCommandCode.isRetrLikeCommand(this.executeTransfer
                .getCommand())) {
            // The command must be launched
            logger.debug("Command launch: {} {}", this.executeTransfer
                    .getCommand(), this.session);
            try {
                this.executeTransfer.getFtpFile().trueRetrieve();
            ...
            logger.debug("Unknown transfer command: {}", this.executeTransfer);
            this.session.getDataConn().getFtpTransferControl()
                    .setEndOfTransfer();
        }
    }
