...
public class FtpDataModeCodec extends FrameDecoder {
	...
	private static final FtpInternalLogger logger =
        FtpInternalLoggerFactory.getLogger(FtpDataModeCodec.class);
	/**
	 * Transfer Mode
	 */
	...
	/**
	 * Inform the Codec that DataNetworkHandler is ready (called from DataNetworkHandler
	 * after setCorrectCodec).
	 *
	 */
	...
	protected Object decode(ChannelHandlerContext ctx,
		...
		// First test if the connection is fully ready (block might be transfered
		...
		if (this.mode == TransferMode.STREAM) {
			...
			// Now we are in BLOCK Mode
		     ...
		     if (this.dataBlock.getByteCount() > 0) {
			     ...
			     this.dataBlock.setBlock(buf.readBytes(this.dataBlock.getByteCount()));
		     }
		...
		// Type unimplemented
		throw new FtpInvalidArgumentException("Mode unimplemented: "+this.mode.name());
	}
	/* (non-Javadoc)
	...
	public void writeRequested(ChannelHandlerContext ctx, MessageEvent evt) throws Exception {
		if (! (evt.getMessage() instanceof FtpDataBlock)) {
			throw new FtpInvalidArgumentException("Incorrect write object: "+evt.getMessage().getClass().getName());
		}
		// First test if the connection is fully ready (block might be transfered
		// by client before connection is ready)
		if (!this.isReady) {
	 ...
	 */
    protected ChannelBuffer encode(FtpDataBlock msg) throws FtpInvalidArgumentException {
		if (msg.isCleared()) {
			return null;
		}
		...
		if (this.mode == TransferMode.STREAM) {
			...
			if (this.structure == TransferStructure.RECORD) {
				ChannelBuffer newbuf = ChannelBuffers.dynamicBuffer(msg.getByteCount());
				int newbyte = 0;
				try {
					while (true) {
			...
			ChannelBuffer newbuf = ChannelBuffers.dynamicBuffer((length > 0xFFFF) ? 0xFFFF+3 : length+3);
			...
			// Work on sub block, ignoring descriptor since it is not the last one
			if (length > 0xFFFF) {
				header[0] = 0;
				header[1] = (byte)0xFF;
				header[2] = (byte)0xFF;
