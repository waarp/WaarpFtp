/**
 * Frederic Bregier LGPL 19 févr. 09 
 * FtpTransferControl.java goldengate.ftp.core.data.handler GoldenGateFtp
 * frederic
 */
...
import goldengate.ftp.core.command.exception.Reply426Exception;
 ...
 * @author frederic
 * goldengate.ftp.core.data.handler FtpTransferControl
 * 
 */
public class FtpTransferControl {
	...
	private static final FtpInternalLogger logger =
        FtpInternalLoggerFactory.getLogger(FtpTransferControl.class);
	/**
	 * Session
	 */
	...
	/**
	 * Blocking step for the Executor 
	 * in order to wait for the end of the command.
	 */
	...
	public void waitForDataNetworkHandlerReady() throws InterruptedException {
		if (! this.isDataNetworkHandlerReady) {
			...
			this.dataNetworkHandlerReady = new FtpFuture();
			logger.debug("Wait for DataNetwork Ready over {}", future.isSuccess());
		}
	}
	/**
	...
	/**
	 * Wait that the new opened connection is ready (same method in {@link FtpDataAsyncConn} from openConnection)
	...
	/**
	 * Wait for the client to be connected (Passive) or Wait for the server to be
	 * connected to the client (Active) (called from {@link FtpCommandUtils} with same method)
	 * @return True if the connection is OK
	 * @throws Reply425Exception 
	 * @throws Reply426Exception 
	 */
		...
		try {
			...
			if (! dataAsyncConn.isStreamFile()) {
				...
				if (dataAsyncConn.isConnected()) {
					...
					this.session.setReplyCode(FtpReplyCode.REPLY_125_DATA_CONNECTION_ALREADY_OPEN,
			...
			if (dataAsyncConn.isPassiveMode()) {
				...
				ClientBootstrap clientBootstrap = 
					this.session.getConfiguration().getFtpInternalConfiguration().getActiveBootstrap();
				// Set the session for the future dataChannel
				this.session.getConfiguration().getFtpInternalConfiguration().setNewFtpSession(
	...
	private void runExecutor() {
		// Unlock Mode Codec
		try {
			this.session.getDataConn().getDataNetworkHandler().unlockModeCodec();
		...
		this.executorService.execute(new FtpTransferExecutor(this.session, this.executingCommand));
	}
	/**
	 * Add a new transfer to be executed. This is to be called from Command after connection is opened and before
	 * answering to the client that command is ready to be executed (for Store or Retrieve like operations).
	 * @param command
	 * @param file
	 */
	...
	/**
	 * Add a new transfer to be executed. This is to be called from Command after connection is opened and before
	 * answering to the client that command is ready to be executed (for List like operations).
	 * @param command
	 * @param list
	...
	/**
	 * Is a command currently executing (called from {@link NetworkHandler} when a message is received to see
	 * if another transfer command is already in execution, which is not allowed)
	 * @return True if a command is currenlty executing
	 */
	 ...
	 */
	private boolean isExecutingRetrLikeTransfer() throws FtpNoTransferException, FtpCommandAbstractException, FtpNoFileException {
		return (FtpCommandCode.isRetrLikeCommand(
				this.getExecutingFtpTransfer().getCommand()) &&
				this.getExecutingFtpTransfer().getFtpFile().isInReading());
	}
	/**
	 ...
	 * @param write True means the message is write back to the control command, false it is only prepapred
	 */
	private void abortTransfer(boolean write) {
		...
		this.session.setReplyCode(
				...
				(current == null ? "Unknown command" : current.toString()));
		if (write) {
	...
	/**
	 * Set the current transfer as finished. Called from {@link FtpTransferExecutor} when a transfer is over.
	 *
	 */
	...
	/**
	 * Called by messageReceived, channelClosed (from {@link DataNetworkHandler}) and trueRetrieve (from {@link FtpFile}) when the transfer is over 
	...
	/**
	 * Clear the FtpTransferControl (called when the data connection must be over like
	 * from clear of {@link FtpDataAsyncConn}, abort from {@link ABOR} or ending
	 * control connection from {@link NetworkHandler}.
	 *
	 */
	public void clear() {
